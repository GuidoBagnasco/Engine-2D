...
void Scene::Draw(Renderer* m_pRender)
{
	...
	if (m_mapaEntity.size() > 0)
	{
		...
		do
		{
			/* DISABLE THIS ! Para no dibujar AABBS!*/
			//m_mIteratorEntity->second->drawAABB(*m_pRender);
...
Entity*Scene::GetEntity(std::string sName)
{
	/*m_mIteratorEntity = m_mapaEntity.begin();
	m_mIteratorEntity = m_mapaEntity.find(sNombre);
	return m_mIteratorEntity->second;*/

	if (m_mapaEntity.count(sName))
		return m_mapaEntity[sName];
	else
		return NULL;
}


TileMap* Scene::GetTileMap(std::string sName){
	if (m_mTileMaps.count(sName))
		return m_mTileMaps[sName];
	else
		return NULL;
}
			...
			for (m_pClsGroupsIterB = (++m_pClsGroupsIterA); m_pClsGroupsIterB != m_pClsGroups.end(); m_pClsGroupsIterB++)
			{
				...
				for (int i = 0; i < m_pEntVectorA->size(); i++)
				{
					...
					for (int j = 0; j < m_pEntVectorB->size(); j++)
					{
						...
						if (m_pEntA->checkCollision(*m_pEntB) != Entity::NoCollision) // me fijo si colisiono
						{
