...
void TileMap::Draw(Renderer* pkRenderer)
{
    for(int i = 0; i < m_vLayers.size(); i++)
        for (int j = 0; j < m_vLayers[i].size(); j++)
            if(m_vLayers[i][j] != NULL) m_vLayers[i][j]->Draw(pkRenderer);
}
...
void TileMap::SetPosition(const float& x, const float& y)
{
    ...
    for (int k = 0; k < m_vLayers.size(); k++) {
        ...
        for (int row = 0 ; row< m_uiRows; row++)
            for(int col = 0; col < m_uiCols; col++, i++)
                if(m_vLayers[k][i] != NULL) m_vLayers[k][i]->setPos( m_vLayers[k][i]->scaleX() * col + fX, m_vLayers[k][i]->scaleY() * -row +fY);
    }
}
...
void TileMap::ScaleMap(float x, float y){
	for(int i = 0; i < m_vLayers.size(); i++)
        for (int j = 0; j < m_vLayers[i].size(); j++)
			if (m_vLayers[i][j] != NULL)
				m_vLayers[i][j]->setScale(x * m_vLayers[i][j]->width, y * m_vLayers[i][j]->height);	
...
Entity::CollisionResult TileMap::CollidesWith(std::string layerName, Entity* entity) {
	if (m_mCollisionLayers.count(layerName) <= 0) return Entity::NoCollision;

	std::vector<Tile*> layer = m_vLayers[m_mCollisionLayers[layerName]];
	std::vector<Tile*>::iterator m_mIterator;

	for (m_mIterator = layer.begin(); m_mIterator != layer.end(); m_mIterator++) {
		if ((*m_mIterator) != NULL) {
			Entity::CollisionResult result = (*m_mIterator)->checkCollision(*entity);
			if (result != Entity::NoCollision) {
				return result;
			}
		}
	}

	return Entity::NoCollision;
}

//-----------------------------------------------------------------

void TileMap::AddCollisionLayer(std::string layerName, int layerNumber) {
	m_mCollisionLayers[layerName] = layerNumber;
}