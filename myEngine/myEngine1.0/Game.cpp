...
Game::~Game() {
	for (m_mIterator = m_map.begin(); m_mIterator != m_map.end(); m_mIterator++) {
...
void Game::AddScene(Scene* newScene) {
	...
	if (newScene->GetName() == ""){
		newScene->SetName("NewScene-" + (m_map.size() + 1));
	}
	m_map[newScene->GetName()] = newScene;
}